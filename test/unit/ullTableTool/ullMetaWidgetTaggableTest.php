...
$t->diag('for write access:');
  ...
  $widget = new ullMetaWidgetTaggable($columnConfig);
  $t->isa_ok($widget, 'ullMetaWidgetTaggable', '__construct() returns the correct object');
  $t->isa_ok($widget->getSfWidget(), 'ullWidgetTaggable', 'returns the correct widget for read access');
  $t->isa_ok($widget->getSfValidator(), 'ullValidatorTaggable', 'returns the correct validator for read access');    