<form class="inline" name="ull_wiki_filter_form_header" method="post" action="<?=url_for('ullWiki/list')?>">
  ...
  <?php if ($sf_user->getAttribute('has_javascript')): ?>
    <?php echo ull_link_to_function(ull_image_tag('search', array('alt' => 'search_header'), null, null, 'ullWiki'), 'document.ull_header_wiki_search_form.submit();', 'ull_js_observer_confirm=true style=margin:0;') ?>
  <?php else: ?>
