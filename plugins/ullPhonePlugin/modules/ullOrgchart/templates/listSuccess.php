<?php echo $breadcrumb_tree ?>

<div class='ull_orgchart'>

  <table cellpadding='0' cellspacing='0'>
    <tbody>
    
      <tr class='ull_orgchart_head'>
        <td>
          <div class='ull_orgchart_box_head'>
            Geschäftsführer
          </div>
        </td>
      </tr>
  
      <tr class='ull_orgchart_subordinates'>
        <td>
          <table cellpadding='0' cellspacing='0'>

            <tr class='ull_orgchart_spacer_row'>
              <td>&nbsp;</td>
              <td class='ull_orgchart_border_right'>&nbsp;</td>
              <td class='ull_orgchart_border_left'>&nbsp;</td>
              <td>&nbsp;</td>
            </tr> 
            
            <tr class='ull_orgchart_boxes_row'>
              <td rowspan='2'>
                <div class='ull_orgchart_box_subordinate'>
                  Assistent1
                </div> 
              </td>
              <td class='ull_orgchart_border_right ull_orgchart_border_double_bottom'>
                <!--  necessary to force width for single column subordinates -->
                <div class='ull_orgchart_spacer_in_between'>&nbsp;</div>
              </td>
              <td class='ull_orgchart_border_left ull_orgchart_border_double_bottom'>
                <div class='ull_orgchart_spacer_in_between'>&nbsp;</div>
              </td>                
              <td rowspan='2'>
                <div class='ull_orgchart_box_subordinate'>
                  Assistent2
                </div>            
              </td>
            </tr>        
            <tr>
              <td class='ull_orgchart_spacer_in_between ull_orgchart_border_right'>&nbsp;</td>
              <td class='ull_orgchart_spacer_in_between ull_orgchart_border_left'>&nbsp;</td>                
            </tr> 
            
            <tr class='ull_orgchart_spacer_row'>
              <td>&nbsp;</td>
              <td class='ull_orgchart_border_right'>&nbsp;</td>
              <td class='ull_orgchart_border_left'>&nbsp;</td>
              <td>&nbsp;</td>
            </tr>              
                      
            <tr class='ull_orgchart_boxes_row'>
              <td rowspan='2'>
                <div class='ull_orgchart_box_subordinate'>
                  Assistent1
                </div> 
              </td>
              <td class='ull_orgchart_border_right ull_orgchart_border_double_bottom'>
                <div class='ull_orgchart_spacer_in_between'>&nbsp;</div>
              </td>
              <td class='ull_orgchart_border_left'>
                <div class='ull_orgchart_spacer_in_between'>&nbsp;</div>
              </td>                
              <td rowspan='2'>&nbsp;</td>
            </tr>        
            <tr>
              <td class='ull_orgchart_spacer_in_between ull_orgchart_border_right'>&nbsp;</td>
              <td class='ull_orgchart_spacer_in_between ull_orgchart_border_left'>&nbsp;</td>              
            </tr>    
            
            <tr class='ull_orgchart_spacer_row'>
              <td>&nbsp;</td>
              <td class='ull_orgchart_border_right'>&nbsp;</td>
              <td class='ull_orgchart_border_left'>&nbsp;</td>
              <td>&nbsp;</td>
            </tr>                               
            
          </table>
        </td>
      </tr>
      
      
      <tr class='ull_orgchart_sub_superiors'>
        <td>
          <table cellpadding='0' cellspacing='0'>
          
            <tr class='ull_orgchart_spacer_row'>
              <td class='ull_orgchart_border_right'>&nbsp;</td>
              <td class='ull_orgchart_border_top ull_orgchart_border_left '>&nbsp;</td>
              <td class='ull_orgchart_border_top ull_orgchart_border_right'>&nbsp;</td>
              <td class='ull_orgchart_border_top ull_orgchart_border_left '>&nbsp;</td>
              <td class='ull_orgchart_border_top ull_orgchart_border_right'>&nbsp;</td>
              <td class='ull_orgchart_border_top ull_orgchart_border_left '>&nbsp;</td>
              <td class='ull_orgchart_border_top ull_orgchart_border_right'>&nbsp;</td>
              <td class='ull_orgchart_border_top ull_orgchart_border_left '>&nbsp;</td>
              <td class='ull_orgchart_border_top ull_orgchart_border_right'>&nbsp;</td>
              <td class='ull_orgchart_border_left'>&nbsp;</td>
            </tr>
            
            <tr class='ull_orgchart_sub_superiors_row'>
              <td colspan="2">
                <div class='ull_orgchart_box_head'>
                  Head of Marketing
                </div>               
              </td>
              <td colspan="2">
                <div class='ull_orgchart_box_head'>
                  Head of IT
                </div>               
              </td>
              <td colspan="2">
                <div class='ull_orgchart_box_head'>
                  Head of fools
                </div>               
              </td>
              <td colspan="2">
              
                <table cellpadding='0' cellspacing='0'>
                  <tbody>
                  
                    <tr class='ull_org_chart_boxes_row head'>
                      <td>
                        <div class='ull_orgchart_box_head'>
                          Head of Superstars
                        </div>
                      </td>
                    </tr>
                
                    <tr class='ull_orgchart_subordinates'>
                      <td>
                          <table cellpadding='0' cellspacing='0'>
              
                            <tr class='ull_orgchart_spacer_row'>
                              <td>&nbsp;</td>
                              <td class='ull_orgchart_border_right'>&nbsp;</td>
                              <td class='ull_orgchart_border_left'>&nbsp;</td>
                              <td>&nbsp;</td>
                            </tr> 
                            
                            <tr class='ull_orgchart_boxes_row'>
                              <td rowspan='2'>
                                &nbsp;
                              </td>
                              <td class='ull_orgchart_border_right'>
                                <div class='ull_orgchart_spacer_in_between'>&nbsp;</div>
                              </td>
                              <td class='ull_orgchart_border_left ull_orgchart_border_double_bottom'>
                                <div class='ull_orgchart_spacer_in_between'>&nbsp;</div>
                              </td>                
                              <td rowspan='2'>
                                <div class='ull_orgchart_box_subordinate'>
                                  Sub Assistent 1
                                </div>            
                              </td>
                            </tr>        
                            <tr>
                              <td class='ull_orgchart_spacer_in_between ull_orgchart_border_right'>&nbsp;</td>
                              <td class='ull_orgchart_spacer_in_between ull_orgchart_border_left'>&nbsp;</td>                
                            </tr>  
                            
                            <tr class='ull_orgchart_spacer_row'>
                              <td>&nbsp;</td>
                              <td class='ull_orgchart_border_right'>&nbsp;</td>
                              <td class='ull_orgchart_border_left'>&nbsp;</td>
                              <td>&nbsp;</td>
                            </tr>        
                            
                            <tr class='ull_orgchart_boxes_row'>
                              <td rowspan='2'>
                                &nbsp;
                              </td>
                              <td class='ull_orgchart_border_right'>
                                <div class='ull_orgchart_spacer_in_between'>&nbsp;</div>
                              </td>
                              <td class='ull_orgchart_border_left ull_orgchart_border_double_bottom'>
                                <div class='ull_orgchart_spacer_in_between'>&nbsp;</div>
                              </td>                
                              <td rowspan='2'>
                                <div class='ull_orgchart_box_subordinate'>
                                  Sub Assistent 2
                                </div>            
                              </td>
                            </tr>        
                            <tr>
                              <td class='ull_orgchart_spacer_in_between'>&nbsp;</td>
                              <td class='ull_orgchart_spacer_in_between'>&nbsp;</td>                
                            </tr> 
                            
                          </table>
                      </td>
                    </tr>
                    
                    
                  </tbody>
                </table>              
              </td>
              <td colspan="2">
                <div class='ull_orgchart_box_head'>
                  Head of Oranges
                </div>               
              </td>
            </tr>            
          
          </table>
        </td>
      </tr>
      
    </tbody>
  </table>
  
</div>


<!-- 
<div class='ull_orgchart'>

  <?php //echo $tree ?>

</div>
 -->

