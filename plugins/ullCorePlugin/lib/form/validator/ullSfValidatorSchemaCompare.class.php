 ...
 */
class ullSfValidatorSchemaCompare extends sfValidatorSchema
{
  const EQUAL              = '==';
  const NOT_EQUAL          = '!=';
  const LESS_THAN          = '<';
   ...
   *  * allow_empty:        Allow one or both fields to be empty. Useful e.g. for date comparison
   *                        when we have a start and end date in a form but the enddate isn't mandatory initially
   *
  ...
  public function __construct($leftField, $operator, $rightField, $options = array(), $messages = array())
  {
    ...
    $this->addOption('allow_empty', false);
  ...
  protected function doClean($values)
  {
    ...
    if ($this->getOption('allow_empty') === true && (empty($leftValue) || empty($rightValue)))
    {
      $valid = true;
    }
